export default function FormRenderer({ data, setData }) {
  ...
  const formData = data?.data ? data.data : data;
      ...
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-3">
          ...
          <span className="ml-auto text-xs bg-gradient-to-r from-blue-500 to-cyan-500 text-white px-3 py-1 rounded-full">
            {Object.values(formData).filter(v => v && v !== "").length}/{formFields.length} fields
          </span>
        </h2>
        ...
        {formFields.map(({ key, label, type, icon, required, options }) => (
          <div key={key} className={type === "select" || key === "passingyear" ? "md:col-span-1" : ""}>
            ...
            {type === "select" ? (
              <select
                value={formData[key] || ""}
                onChange={(e) => setData({ ...formData, [key]: e.target.value })}
      ...
      <div className="mt-6 pt-6 border-t border-slate-200">
        <div className="flex items-center justify-between text-sm">
          <span className="text-slate-600">
            <strong>{Object.values(formData).filter(v => v && v !== "").length}</strong> of <strong>{formFields.length}</strong> fields completed
          </span>
            ...
            {formFields.map(({ key }) => (
              <div
                ...
                className={`w-2 h-2 rounded-full transition-all ${
                  formData[key] && formData[key] !== "" ? "bg-green-500" : "bg-slate-300"
                }`}
              ></div>
            ))}
          </div>
        ...
        <div className="mt-3 w-full bg-slate-200 rounded-full h-2">
          <div
            ...
            style={{
              width: `${(Object.values(formData).filter(v => v && v !== "").length / formFields.length) * 100}%`,
            }}
          ></div>
        </div>
      </div>
